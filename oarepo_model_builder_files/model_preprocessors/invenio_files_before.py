...
from oarepo_model_builder.model_preprocessors.invenio import process_pid_type
...
class InvenioModelFilesBeforePreprocessor(ModelPreprocessor):
    ...
    def transform(self, schema, settings):
        ...
        parent_record_prefix = camel_case(model["package"].rsplit(".", maxsplit=1)[-1])
        files.setdefault("record-prefix", f"{parent_record_prefix}File")
        files.setdefault("record-permissions-class", model["record-permissions-class"])
        ...
        files.setdefault(
            "record-parent-class", "invenio_records_resources.records.api.FileRecord"
        )
        ...
        files.setdefault("pid-type", process_pid_type(model["model-name"] + "File"))
